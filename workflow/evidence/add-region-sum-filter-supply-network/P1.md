...
## Key Decisions
- 3 文件：types.ts 字段 + orchestration 默认值/透传（空数组→undefined）+ control-bar computed/模板
- 数据源 enumStore.customerRegionSumFilterOptions，传参 saleZoneSumIn（数组，同 saleZoneIn 模式）
