...
runAs: subagent
allowed-tools: read_file, grep, glob, ls, code_index, edit_file, multi_edit, write_file, bash, lsp_diagnostics
---
...
① 解析任务指令
② 读取涉及文件（read_file）
③ 如果需求涉及领域组件/API，先查 AGENTS.md 路由表对应的 skill 文档
④ 实现改动（最小改动原则）
⑤ 运行 {{PM}} -s run {{TWEAK_SCRIPT}} 验证
...
- `bash` — 运行 `{{PM}} -s run {{TWEAK_SCRIPT}}` 等验证命令
