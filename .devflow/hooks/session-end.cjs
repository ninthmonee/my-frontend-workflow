...
/**
 * session-end.cjs — SessionEnd Hook: DevFlow 提醒 + 清理
 *
 * 接收 payload: { event, cwd }
 * - 检查 DevFlow 是否完成 → 完成则确认，未完成则输出提醒到 stderr
 * - 仅在流程 DONE 时清理临时文件
 */
...
if (ctx.phase >= 5) {
  console.log(`✅ DevFlow [${name}] 流程完成。`);
} else {
  console.log(
    `⚠️ DevFlow [${name}] 尚未完成 (Phase ${ctx.phase}/4)。下次打开此会话将自动恢复。`,
  );
}

// ── 仅在 DONE 时清理临时文件 ──────────────────────────
if (ctx.phase >= 5) {
  const harnessDir = path.resolve('.devflow/harness');
