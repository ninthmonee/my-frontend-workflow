...
- [ ] 是否运行了 pnpm run check:type 且 exit code = 0？
- [ ] 是否运行了 lint（按需）并确认无错误？
- [ ] 失败时是否委派了独立的 `p2-verifier` 子 Agent 分析错误？（skill: `.reasonix/skills/p2-verifier.md`）
...
```bash
pnpm -s run harness:gate-verify -- --id <change>   # 校验 P0/P1/P2 Gate 完整性
cat workflow/harness/state.json                     # 查看当前全量状态
```
